...
if(isset($_GET['id'])){
	...
	$cek = mysqli_query($db,"SELECT id_user FROM user WHERE id_user='$id'") or die(mysqli_error());
	...
	if(mysqli_num_rows($cek) == 0){
		...
		$del = mysqli_query($db,"DELETE FROM user WHERE id_user='$id'");
		...
		if($del){
			...
			echo '<script>window.alert("Data berhasil dihapus")
				...
				window.location="user.php"</script>';		//Pesan jika proses delete sukses
